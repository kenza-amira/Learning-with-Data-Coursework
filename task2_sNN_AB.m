...
%X = [2,5;1,1.5;2,3;5,1];
 ...
 %lower triangle. Notice that for all the upcoming weights, I have
 %multiplied by 10^7 (10e6) to squeeze the sigmoid function into a step
 %function
 W_B_UP = 10e6*[1;0.0416;-0.2819;1;-0.7469;0.8346;-1;0.3553;0.3096];
 W_B_DW = 10e6*[1;-0.5294;0.1990;-1;0.8891;0.3469;1;-0.3553;-0.3096];
 ...
 for i = (1:size(X,1))
   %For Polygon B
   hiddenU = [hiddenU; task2_sNeuron(layer1BU,X(i,:))];
   hiddenD = [hiddenD; task2_sNeuron(layer1BD,X(i,:))];
   %For polygon A
   hiddenA = [hiddenA; task2_sNeuron(layer1A,X(i,:))];
 end
 ...
 for i = 1:size(hiddenU,1)
   outU = [outU; task2_sNeuron(layer2B,hiddenU(i,:))];
   outD = [outD; task2_sNeuron(layer2B,hiddenD(i,:))];
 end
 %Same as above but for Polygon A
 resultsA = [];
 for i = 1:size(hiddenA,1)
   resultsA = [resultsA; task2_sNeuron(layer2A,hiddenA(i,:))];
 end
 ...
 %triangles so we design an OR gate only here the threshold is 0.5
 resultsB = [];
  for j = 1:size(concatOut,1)
      if concatOut(j,1) > 0.5  | concatOut(j,2) > 0.5 
          resultsB = [resultsB;1];
      else
          resultsB = [resultsB;0];
      end
 ...
 %outside of B, so we look for a <= 0.5 in A and >0.5 in B
 Y = [];
 for x = 1:size(results,1)
     if results(x,1) <= 0.5 & results(x,2) > 0.5 
         Y = [Y;1];
     else
         Y = [Y;0];
     end
